...
<div
	...
	role="button"
	tabindex="0"
	on:click={() => fileType === 'image' && onEdit(file)}
	...
	<div class="thumb-box">
		...
		{#if fileType === 'image'}
			...
			<div class="overlay">
				<button
					class="overlay-btn"
					on:click|stopPropagation={previewImage}
					aria-label="Preview imagen">Preview</button
				>
				<button
					class="overlay-btn"
					on:click|stopPropagation={() => onEdit(file)}
					aria-label="Editar imagen">Edit</button
				>
			</div>
